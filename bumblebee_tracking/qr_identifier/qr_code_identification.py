...
def identify_qr_code(image_path, bounding_box=None):
    ...
    for tag in tags:
        ...
        adjusted_corners = [(corner[0] + crop_offset_x, corner[1] + crop_offset_y) for corner in tag.corners]
...
def visualize_detection(image_path, bounding_boxes, detected_tags, save_path=None):
    ...
    for i, bbox in enumerate(bounding_boxes):
        try:
            ...
            if len(coords) == 4:
                ...
                # Add label
                ax.text(x1, y1-5, f'BBox {i+1}', color=color, fontsize=10, 
    ...
    for tag in detected_tags:
        ...
        # Add tag ID label
        ax.text(center[0], center[1]+15, f'ID: {tag_id}', 
    ...
    legend_elements.append(plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='red', 
                                     markersize=8, label='Detected Tags'))
...
def process_images_from_csv(csv_file_path):
    ...
    for image_path, rows in image_data.items():
        ...
        for row in rows:
            ...
            if (row.get('bbox1_x') and row.get('bbox1_y') and 
                row.get('bbox1_w') and row.get('bbox1_h') and
                row.get('bbox1_x') != '' and row.get('bbox1_y') != '' and
                row.get('bbox1_w') != '' and row.get('bbox1_h') != ''):
                try:
                    x = int(row['bbox1_x'])
                    y = int(row['bbox1_y'])
                    w = int(row['bbox1_w'])
            ...
            if (row.get('bbox2_x') and row.get('bbox2_y') and 
                row.get('bbox2_w') and row.get('bbox2_h') and
                row.get('bbox2_x') != '' and row.get('bbox2_y') != '' and
                row.get('bbox2_w') != '' and row.get('bbox2_h') != ''):
                try:
                    x = int(row['bbox2_x'])
                    y = int(row['bbox2_y'])
                    w = int(row['bbox2_w'])
            ...
            while True:
                ...
                if (row.get(bbox_x_key) and row.get(bbox_y_key) and 
                    row.get(bbox_w_key) and row.get(bbox_h_key) and
                    row.get(bbox_x_key) != '' and row.get(bbox_y_key) != '' and
                    row.get(bbox_w_key) != '' and row.get(bbox_h_key) != ''):
                    try:
